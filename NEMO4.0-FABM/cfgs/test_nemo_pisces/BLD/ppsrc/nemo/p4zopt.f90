...
CONTAINS
   ...
   SUBROUTINE p4z_opt( kt, knt )
      ...
      IF( l_trcdm2dc ) THEN                     !  diurnal cycle
         ...
         !---- Mokrane------
          IF( lk_iomput .AND.  knt == nrdttrc ) THEN
            CALL iom_put( "qsr_mean"  , zqsr_corr(:,:) * tmask(:,:,1) )  ! Light at theeuphotic depth
            CALL iom_put( "pe1"  , ze1(:,:,:) * tmask(:,:,:) )
            CALL iom_put( "pe2"  , ze2(:,:,:) * tmask(:,:,:) )
            CALL iom_put( "pe3"  , ze3(:,:,:) * tmask(:,:,:) )
            CALL iom_put( "PARDM", etot_ndcy(:,:,:) * tmask(:,:,:) )  !Photosynthetically Available Radiation
         ENDIF
         !--------------------------
      ...
      IF( lk_iomput .AND.  knt == nrdttrc ) THEN
         ...
         CALL iom_put( "Heup_01" , heup_01(:,:  ) * tmask(:,:,1) )  ! Added by Mokrane
         ...
         CALL iom_put( "zchl" , zchl3d(:,:,:) * tmask(:,:,:) ) ! total chlorophyll
         CALL iom_put( "qsr"  , ( qsr(:,:) / ( 1.-fr_i(:,:) + rtrn ) ) * tmask(:,:,1) )  ! Light at the euphotic depth
         CALL iom_put( "pqsr100"  ,  zqsr100(:,:) * tmask(:,:,1) )  !
      ENDIF
      !
